...
class TemporalJitter(object):
    """
    Randomly shifts frames in the video by up to max_shift frames.
    Args:
...
class SpeedChange(object):
    """
    Changes the speed of the video by resampling frames.
    Args:
        speed_factor (float): Factor to change speed (0.8x to 1.2x typical).
    """
    def __init__(self, speed_factor=1.0):
        self.speed_factor = speed_factor
    def __call__(self, clip):
        n_frames = len(clip)
        new_n_frames = int(n_frames / self.speed_factor)
