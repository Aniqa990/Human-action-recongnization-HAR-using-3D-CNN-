...
parser.add_argument('--resume_checkpoint', type=str, default=None,
                    help='Path to best_model.pth to warm-start fold weights.')
...
def train_fold(fold_idx: int, train_indices: list,
               val_indices: list, all_samples: list,
               resume_ckpt_path=None) -> float:
...
# ── Load checkpoint weights if resuming ───────────────────────────────────
    if resume_ckpt_path and os.path.exists(resume_ckpt_path):
        print(f"\n  Loading checkpoint: {resume_ckpt_path}")
        ckpt = torch.load(resume_ckpt_path, map_location=device)
        ckpt_fold = ckpt.get('fold', None)
        model.load_state_dict(ckpt['state_dict'])
        saved_acc = ckpt.get('val_acc', 0.0)
        print(f"  ✅ Weights loaded (from fold {ckpt_fold}, "
              f"val_acc {saved_acc*100:.2f}%)")
        # If checkpoint is from a different fold, only use weights —
        # reset everything else so this fold trains fully from scratch
        # with a warm-started backbone instead of random Kinetics init
        if ckpt_fold != fold_idx:
            print(f"  Different fold — using weights only, "
                  f"training all {args.n_epochs} epochs fresh.")
    elif resume_ckpt_path:
        print(f"  [WARN] Checkpoint not found: {resume_ckpt_path}")
...
if __name__ == '__main__':
    ...
    for fold_i, (tr_idx, vl_idx) in enumerate(splits):
        ...
        if fold_i < args.resume_from_fold:
            # Try to load saved accuracy so final summary is complete
            ...
            if os.path.exists(best_path):
                ...
                print(f"[Skip fold {fold_i} — recovered saved acc {saved_acc*100:.2f}%]")
            else:
                print(f"[Skip fold {fold_i} — no checkpoint found, acc unknown]")
            continue
        ...
        # Only pass resume checkpoint for the first fold we actually run
        ckpt_path = (args.resume_checkpoint
                     if fold_i == args.resume_from_fold else None)
